...
import com.criteo.hadoop.garmadon.hdfs.writer.CloseableBiConsumer;
...
public class ExpiringConsumerTest {
    ...
    public void expiredAfterTooManyMessages() throws IOException {
        CloseableBiConsumer<Integer, Offset> innerMock = mock(CloseableBiConsumer.class);
        ExpiringConsumer<Integer> consumer = new ExpiringConsumer<>(innerMock, Duration.ofMinutes(10), 3);
    ...
    public void expiredAfterTooMuchIdleTime() throws InterruptedException, IOException {
        CloseableBiConsumer<Integer, Offset> innerMock = mock(CloseableBiConsumer.class);
        ExpiringConsumer<Integer> consumer = new ExpiringConsumer<>(innerMock, Duration.ofSeconds(1), 10);
    ...
    public void expiredFromTheStart() throws InterruptedException {
        ExpiringConsumer<Integer> noTimeConsumer = new ExpiringConsumer<>(null, Duration.ZERO, 10);
        Thread.sleep(1);
        Assert.assertTrue(noTimeConsumer.isExpired());

        ExpiringConsumer<Integer> noMessageConsumer = new ExpiringConsumer<>(null, Duration.ofMinutes(10), 0);
        Assert.assertTrue(noMessageConsumer.isExpired());

        ExpiringConsumer<Integer> allZerosConsumer = new ExpiringConsumer<>(null, Duration.ZERO, 0);
        Thread.sleep(1);
    ...
    public void messagesArePassedToInnerWriter() throws IOException {
        CloseableBiConsumer<Integer, Offset> innerMock = mock(CloseableBiConsumer.class);
        ExpiringConsumer<Integer> consumer = new ExpiringConsumer<>(innerMock, Duration.ZERO, 5);
        List<Offset> offsets = Arrays.asList(buildOffset(11), buildOffset(12), buildOffset(13));

        for (int i = 0; i < 3; i++) {
            consumer.write(i, offsets.get(i));
        }

        InOrder inOrder = inOrder(innerMock);
        for (int i = 0; i < 3; i++) {
    ...
    public void closeWithNoMessage() throws IOException {
        CloseableBiConsumer<Integer, Offset> innerMock = mock(CloseableBiConsumer.class);
        ExpiringConsumer<Integer> consumer = new ExpiringConsumer<>(innerMock, Duration.ZERO, 10);
    ...
    private static Offset buildOffset(long offset) {
        Offset offsetMock = mock(Offset.class);
