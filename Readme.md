...
That stream of events is used to build end-users and SRE teams dashboards, cluster resources usage optimisation, datalineage, etc...
...
Garmadon integrates with any JVM based Yarn framework (mapreduce v1, v2, spark, flink, hive, etc... but you won't get information directly from python containers for instance) and has specific extensions for **Spark 2**.
...
Some of the load is predictible (and tunable) but a big part of the load is also very depending on the number of container you run, their life span, the framework you use, etc...
...
Your Elastic Search cluster has to be configured as a datasource for Grafana.
...
If you don't select a specific containers, the following panels will display Max value et average.
That can be usefull if you want to check a threshold on a lot of containers. 
...
**If not, the application will not be selectable in the drop-down list.**
...
That helps uniformizing HDFS metrics between preprod and prod environments.
...
For now, Garmadon-forwarder and Garmadon-reader exposes metrics that can be fetched by promtheus.
